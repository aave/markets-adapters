...
import "../tokens/ERC20Uniswap.sol";
import "../interfaces/IERC20Uniswap.sol";
import "../interfaces/IUniswapExchange.sol";


contract UniswapExchange is ERC20Uniswap {
  ...
  IERC20Uniswap token;                // address of the ERC20 token traded on this contract
  ...
  function setup(address token_addr) public {
    require( 
      address(token) == address(0) && token_addr != address(0), 
      "INVALID_ADDRESS"
    );
    token = IERC20Uniswap(token_addr);
 ...
 /**
  ...
  {
    address payable exchange_addr = address(this);
  ...
  function addLiquidity(uint256 min_liquidity, uint256 max_tokens, uint256 deadline) public payable returns (uint256) {
    ...
    if (total_liquidity > 0) {
      ...
      require(address(token) != address(0) && msg.value >= 1000000000, "INVALID_VALUE");
